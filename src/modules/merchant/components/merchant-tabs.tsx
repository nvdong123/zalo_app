...
export function MerchantTabs({ activeTab }: { activeTab: MenuType }) {
  ...
  return (
    ...
    <BottomNavigation.Item key="callnow" label="Gọi ngay" icon={<IconPhoneCall />} onClick={() => handleTabChange('callnow')}/>
    <BottomNavigation.Item key="promotion" label="Ưu đãi" icon={<IconPercentage />} onClick={() => handleTabChange('promotions')}/>
    <BottomNavigation.Item key="homepage" label="Trang chủ" icon={<IconHomePage />} onClick={() => handleTabChange('homepage')}/>
    <BottomNavigation.Item key="simplezalo" label="Tin nhắn" icon={<IconSimpleZalo />} onClick={() => handleTabChange('inbox')}/>
    <BottomNavigation.Item key="direction" label="Chỉ đường" icon={<IconDirection />} onClick={() => handleTabChange('direction')}/>
    </BottomNavigation>
  )
}
